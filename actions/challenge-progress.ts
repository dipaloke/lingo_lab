...
export const upsertChallengeProgress = async (challengeId: number) => {
  ...
  const existingChallengeProgress = await db.query.challengeProgress.findFirst({
    where: and(
      //and combines multiple eq together
  ...
  //updating challenge progress for correct answer
  if (isPractice) {
    await db
    ...
    //update userProgress for correct answer, increase 1 heart max 5 increase points by 10
    await db
      .update(userProgress)
      .set({
  ...
  //when user came to practice first time & selected correct answer
